...
# I.MX6 eMMC 启动系统 烧写脚本
# ./imx6-emmc-boot.sh /dev/mmcblk1
...
# 打印用法
f_usage() {
	# sudo ./$(basename $1) /dev/mmcblk1
	func_echo -e "用法示例: $0 /dev/mmcblk1 "
	exit 1
}
...
if [ $device == "/dev/mmcblk0" ]; then
	func_echo -e "请不要选择 /dev/mmcblk0 设备"
	f_usage
fi
...
uninstall_before_formatting() {
	local uninstall="$(ls -1 ${device}p? 2>/dev/null)"
	if [ -n "${uninstall}" ]; then
...
func_echo "
 ...
 第一个分区 为 32M 用来存放 设备树 与 内核镜像文件，因为设备树与内核都比较小，不需要太大的空间
 第二个分区 为 eMMC 的总大小 -32M, 用来存放 根文件系统 "
...
if [ -b ${device}p1 ]; then
	...
	func_execute_sudo mkfs.vfat -F 32 -n "boot" ${device}p1
	# func_execute mkfs -V -t vfat -F 32 -n "boot" ${device}p1
else
	func_echo -e "找不到 ${device}p1: eMMC boot 分区"
	exit 11
fi
...
if [ -b ${device}p2 ]; then
	...
	func_execute_sudo mkfs.ext4 -F -L "rootfs" ${device}p2
	# func_execute mkfs -V -t ext4 -F -L "rootfs" ${device}p2
else
	func_echo -e "找不到 ${device}p2: eMMC rootfs 分区"
	exit 12
fi
...
# 烧写 u-boot.imx 到 emmc boot0 分区
func_echo "烧写 ${Uboot} 到 ${device}boot0 "

func_echo "烧写前，使能写 mmcblk1boot0"
${Sudo} echo 0 >/sys/block/${device##*/}boot0/force_ro # 使能写
# note: chip first 1024 bytes stored disk-partition table discription.
func_execute_sudo dd bs=1024 seek=1 conv=fsync if=${Uboot} of=${device}boot0
func_execute_sudo dd bs=1024 seek=500 conv=fsync if=${Uboot} of=${device}boot0
func_echo "烧写完，关闭写 mmcblk1boot0"
${Sudo} echo 1 >/sys/block/${device##*/}boot0/force_ro # 关闭写

################################################################################

f_sdk() {
	if [ -d "/tmp/sdk" ]; then
		func_execute_sudo rm -rf /tmp/sdk
	fi
}
f_sdk
...
sdk_path=/tmp/sdk/$$/boot
func_mkdir 777 ${sdk_path}

# 烧写内核与设备树到 emmc mmcblk1p1
func_echo "复制 设备树 与 Linux 内核镜像文件 到 ${device}p1 : emmc boot 区"

echo "挂载 ${device}p1 ---- ${sdk_path}"
func_execute_sudo mount ${device}p1 ${sdk_path}
...
sdk_path=/tmp/sdk/$$/rootfs
func_mkdir 777 ${sdk_path}

func_echo "解压 根文件系统 压缩包 到 ${device}p2 : emmc rootfs 区"

echo "挂载 ${device}p2"
func_execute_sudo mount ${device}p2 ${sdk_path}/
...
# 使能启动分区
# sudo apt install -y mmc-utils  -f
# func_execute_sudo mmc bootpart enable 1 1 /dev/mmcblk1
echo "enable boot partion 1 to boot"
func_execute_sudo mmc bootpart enable 1 1 ${device}
